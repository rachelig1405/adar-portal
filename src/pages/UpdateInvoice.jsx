...
export default function UpdateInvoice({ onClose }) {
    ...
    async function loadData() {
      ...
      try {
        const [ordersResponse] = await Promise.all([
          fetch(`${API_URL}/api/orders/filter_by_status?status=בבדיקה`),
  ...
  async function submit(event) {
    ...
    try {
      ...
      if (!response.ok) {
        let message = "שגיאה בעדכון חשבונית";
      ...
      alert("עדכון חשבונית נשמר בהצלחה");
      onClose();
    } catch (submitError) {
      console.error(submitError);
      setError(submitError.message || "שגיאה בעדכון חשבונית");
    } finally {
      setSaving(false);
    }
  }
